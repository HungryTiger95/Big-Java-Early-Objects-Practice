...
public class BankData
{
	...
	/**
	 * Prints out all the the integer and double values, even those that were
	 * removed
	 * @throws IOException
	 */
	public void printDat() throws IOException
	{
		for(int i = 0; i < size(); i++)
		{
			this.file.seek(i * RECORD_SIZE);
			System.out.print("[INT: " + this.file.readInt() + "][DOUBLE: " + this.file.readDouble() + "]");
		}
	}
	...
	public void deleteRecord(int n) throws IOException
	{
		...
		// Get the last element and set this record to that one
		this.file.seek((size() - 1) * RECORD_SIZE);
		int accountNumber = this.file.readInt();
		double balance = this.file.readDouble();
		
		this.file.seek(n * RECORD_SIZE);
		this.file.writeInt(accountNumber);
		this.file.writeDouble(balance);
		
		this.file.seek((size() - 1) * RECORD_SIZE);
		this.file.writeInt(0);
		this.file.writeDouble(0);
		
		this.file.setLength(this.file.length() - (1 * RECORD_SIZE));
	}
